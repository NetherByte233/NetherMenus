...
class NetherMenus extends PluginBase {
    ...
    public function onEnable(): void {
        ...
        // Setup InventoryUI virion
        try {
            InventoryUI::setup($this);
        } catch (\Exception $e) {
            $this->getLogger()->error("Failed to setup InventoryUI: " . $e->getMessage());
        ...
        $this->getLogger()->info("NetherMenus has been enabled!");
    }
            ...
            if (isset($guiData['open_command']) && is_array($guiData['open_command'])) {
                foreach ($guiData['open_command'] as $command) {
                    ...
                    if (!empty($command)) {
                        ...
                        // Only register if not already registered by another plugin
                        if ($commandMap->getCommand($cmdName) === null) {
                            ...
                            $this->getLogger()->warning("Command '$cmdName' is already registered by another plugin and will be skipped");
                        }
                    }
                }
            }
